...
@module.route('/v1/data/summary/enhanced-stream-restoration/<int:practice_id>', methods=['OPTIONS'])
def summary_bioretention_options(practice_id):
...
@module.route('/v1/data/summary/enhanced-stream-restoration/<int:practice_id>', methods=['GET'])
@oauth.require_oauth()
def summary_bioretention_get(oauth_request, practice_id):
    ...
    if check_roles('grantee', authorization.roles):
        if (authorization.id == resource.creator_id) or \
           ...
           (resource.site.creator_id == authorization.id)):
            ...
            logger.warning('User %d %s access failed Enhanced Stream '
                           'Restoration Summary' % (authorization.id,
                                           'grantee'))
            abort(401)
        ...
        logger.info('User %d accessed Enhanced Stream Restoration Summary '
