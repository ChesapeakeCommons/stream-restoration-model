...
@module.route('/v1/data/summary/instream-habitat/<int:practice_id>', methods=['OPTIONS'])
...
@module.route('/v1/data/summary/instream-habitat/<int:practice_id>', methods=['GET'])
@oauth.require_oauth()
    ...
    if check_roles('grantee', authorization.roles):
        if (authorization.id == resource.creator_id) or \
           ...
           (resource.site.creator_id == authorization.id)):
            ...
            logger.warning('User %d %s access failed Practice '
        ...
        logger.info('User %d accessed Practice  Summary '
